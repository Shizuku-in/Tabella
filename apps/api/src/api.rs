...
use crate::{
    AppState, auth,
    dto::{
        AuthUserResponse, DownloadJobRequest, HealthResponse, ImageListItem, ImageSort,
        ListImagesQuery, ListImagesResponse, LoginRequest, Rating, SessionUser,
        TagSuggestQuery, UserRole,
    },
};
...
async fn list_images(
    ...
    for row in rows {
        ...
        items.push(ImageListItem {
            ...
            original_url: (!row.original_path.is_empty()).then(|| format!("/media/{}", row.original_path)),
...
mod tests {
    ...
    use super::{
        ImageListRow, decode_image_cursor, encode_image_cursor, sanitize_upload_path,
    };
