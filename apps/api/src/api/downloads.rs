...
async fn create_download_job(
    ...
    if request.image_ids.is_empty() {
        return Err(ApiError::bad_request("invalid_request", "No images selected"));
    }

    if request.image_ids.len() > settings.max_download_images {
        return Err(ApiError::bad_request(
            "limit_exceeded",
            format!("Cannot download more than {} images at once.", settings.max_download_images),
        ));
    }
    ...
    if records.is_empty() {
        return Err(ApiError::bad_request("invalid_request", "Selected images not found"));
    }
    ...
    for record in &records {
        ...
        let original_filename: String = sqlx::Row::try_get(record, "original_filename").unwrap_or_default();
    ...
    if total_bytes as u64 > settings.max_download_total_bytes {
        return Err(ApiError::bad_request(
            "limit_exceeded",
            format!("Total size exceeds the maximum limit of {} bytes.", settings.max_download_total_bytes),
        ));
    }

    let job_id = Uuid::new_v4();
    let expires_at = OffsetDateTime::now_utc() + time::Duration::hours(settings.download_retention_hours as i64);
...
async fn get_download_job(
    ...
    if record_user_id != user.id {
        return Err(ApiError::unauthorized("unauthorized", "You can only view your own download jobs"));
    }

    Ok(Json(DownloadJobResponse {
        id: sqlx::Row::try_get(&record, "id").unwrap(),
        status: sqlx::Row::try_get::<Option<String>, _>(&record, "status").unwrap().unwrap_or_else(|| "unknown".to_string()),
...
async fn download_job_file(
    ...
    if record_user_id != user.id {
        return Err(ApiError::unauthorized("unauthorized", "You can only download your own files"));
    }

    let record_status: Option<String> = sqlx::Row::try_get(&record, "status").unwrap();
    if record_status.as_deref() != Some("completed") {
        return Err(ApiError::bad_request("invalid_state", "The download job is not completed yet"));
    }

    let file_path: Option<String> = sqlx::Row::try_get(&record, "file_path").unwrap();
    let file_path = file_path.ok_or_else(|| ApiError::internal(anyhow::anyhow!("File path is missing")))?;
    ...
    response.headers_mut().insert(header::CONTENT_TYPE, HeaderValue::from_static("application/zip"));
    response.headers_mut().insert(header::CONTENT_DISPOSITION, HeaderValue::from_str(&content_disposition).unwrap());
