...
| `include_tags`   | CSV 或重复参数                    | AND 过滤：图片必须包含所有列出的标签                    |
| `exclude_tags`   | CSV 或重复参数                    | AND 过滤：图片不得包含任何列出的标签                    |
| `favorites_only` | boolean                           | 仅返回当前用户的收藏                                    |

同时提供 `rating` 和 `max_rating` 时，二者取交集（AND）。提供 `max_rating=explicit` 可放宽上限；提供 `rating=explicit` 则从指定的精确集合中选取。标签过滤和 `favorites_only` 会进一步缩小结果集。
